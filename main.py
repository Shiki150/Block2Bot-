...
S = {
    "fr": {
        ...
        "lang_same":"Le bot est déjà en français !",
    },
    "en": {
        ...
        "lang_same":"Bot is already in English!",
    }
}
...
async def announce(ctx, channel: discord.TextChannel, *, message: str):
    e = discord.Embed(description=message, color=BOT_COLOR,
                      timestamp=datetime.now(timezone.utc))
    ...
    await ctx.send(embed=mk_embed("✅ Annonce envoyée",
        f"Message posté dans {channel.mention}"), delete_after=4)
...
COMPLIMENTS = [
    ...
    "Le serveur est bien mieux avec toi 💫",
    "Tu es une source d'inspiration ✨",
    "Ta présence illumine ce Discord 🌸",
    "Tu es quelqu'un de vraiment formidable 🎉",
    "On a de la chance de t'avoir ici 🍀",
]
...
async def roll(ctx, maximum: int = 100):
    if maximum < 2:
        return await ctx.send(embed=mk_embed("❌ Erreur",
            "Le max doit être ≥ `2`.", 0xE74C3C), delete_after=5)
...
async def _8ball(ctx, *, question: str = None):
    ...
    await ctx.send(embed=mk_embed("🎱 8-Ball",
        f"**❓ {question}**\n\n{random.choice(BALL)}"))
...
async def mock(ctx, *, text: str = None):
    ...
    await ctx.send(embed=mk_embed("🤪 Mock", mocked))
...
async def bot_en(ctx):
    global LANG
    if LANG == "en":
        return await ctx.send(embed=mk_embed("🌍 Language",
            tr("lang_same"), 0xE67E22), delete_after=4)
    LANG = "en"
...
async def bot_fr(ctx):
    global LANG
    if LANG == "fr":
        return await ctx.send(embed=mk_embed("🌍 Langue",
            tr("lang_same"), 0xE67E22), delete_after=4)
    LANG = "fr"
...
async def help_cmd(ctx):
    if LANG == "en":
        ...
        e.set_thumbnail(url=bot.user.display_avatar.url)
        e.add_field(name="🤖 AI", inline=False,
            value="`!ask <question>` — Ask the AI\n`!ia` — Auto-reply mode *(admin)*")
        e.add_field(name="🎮 Fun", inline=False,
            value="`!helpfun` — All fun commands")
        e.add_field(name="🌍 Utilities", inline=False,
            value="`!helputil` — All utility commands")
        if is_mod(ctx):
            e.add_field(name="🛡️ Moderation", inline=False,
                value="`!helpmodo` — All staff commands")
        e.add_field(name="🌐 Language", inline=False,
            value="`!botfr` — Passer en Français 🇫🇷")
    else:
        e = discord.Embed(title="📖 Block2Bot",
            description=f"Préfixe : `!`  ·  [Serveur Support]({SUPPORT_URL})",
            color=BOT_COLOR, timestamp=datetime.now(timezone.utc))
        e.set_thumbnail(url=bot.user.display_avatar.url)
        e.add_field(name="🤖 IA", inline=False,
            value="`!ask <question>` — Poser une question à l'IA\n`!ia` — Mode auto *(admin)*")
        e.add_field(name="🎮 Fun", inline=False,
            value="`!helpfun` — Toutes les commandes fun")
        e.add_field(name="🌍 Utilitaires", inline=False,
            value="`!helputil` — Tous les utilitaires")
        if is_mod(ctx):
            e.add_field(name="🛡️ Modération", inline=False,
                value="`!helpmodo` — Toutes les commandes staff")
        e.add_field(name="🌐 Langue", inline=False,
            value="`!boten` — Switch to English 🇬🇧")
...
async def help_fun(ctx):
    if LANG == "en":
        e = discord.Embed(title="🎮 Fun Commands", description="All fun commands",
                          color=BOT_COLOR, timestamp=datetime.now(timezone.utc))
        e.add_field(name="🪙 !coin",                   value="Flip a coin — Heads or Tails",                      inline=False)
        e.add_field(name="🎲 !roll [max]",             value="Random number between 1 and max (default: 100)",     inline=False)
        e.add_field(name="🎱 !8ball <question>",       value="Ask the magic 8-Ball anything 🔮",                   inline=False)
        ...
        e.add_field(name="🤪 !mock <text>",            value="tUrNs AnY tExT iNtO mOcK fOrMaT",                   inline=False)
        e.add_field(name="🎯 !choose opt1 | opt2",    value="Let the bot pick between your options",              inline=False)
        ...
        e = discord.Embed(title="🎮 Commandes Fun", description="Toutes les commandes fun",
                          color=BOT_COLOR, timestamp=datetime.now(timezone.utc))
        e.add_field(name="🪙 !coin",                  value="Pile ou face — à toi de jouer !",                    inline=False)
        e.add_field(name="🎲 !roll [max]",            value="Nombre aléatoire entre 1 et max (défaut : 100)",     inline=False)
        ...
        e.add_field(name="🎯 !choose opt1 | opt2",   value="Le bot choisit entre plusieurs options",             inline=False)
...
async def help_util(ctx):
    if LANG == "en":
        e = discord.Embed(title="🌍 Utility Commands", description="All utility commands",
                          color=BOT_COLOR, timestamp=datetime.now(timezone.utc))
        e.add_field(name="🤖 !ask <question>",      value="Ask the AI (memory + auto web search)",                inline=False)
        e.add_field(name="🟢/🔴 !ia",              value="Enable/disable AI auto-reply in channel *(admin)*",   inline=False)
        e.add_field(name="🏓 !ping",                value="Check bot latency — 🟢 good · 🟡 ok · 🔴 bad",       inline=False)
        e.add_field(name="⏰ !time [country]",      value="Current time — france, usa, japan, uk, maroc...",      inline=False)
        e.add_field(name="👤 !userinfo [@member]",  value="Detailed information about a member",                  inline=False)
        ...
        e.add_field(name="🏷️ !roleinfo @role",      value="Role details — members, color, permissions",           inline=False)
        e.add_field(name="🌐 !boten / !botfr",      value="Switch bot language between English and French",       inline=False)
    else:
        e = discord.Embed(title="🌍 Commandes Utilitaires", description="Tous les utilitaires",
                          color=BOT_COLOR, timestamp=datetime.now(timezone.utc))
        ...
        e.add_field(name="👤 !userinfo [@membre]", value="Infos complètes sur un membre — ID, rôles, dates",    inline=False)
        e.add_field(name="🏰 !serverinfo",         value="Stats serveur — membres, salons, rôles, proprio",     inline=False)
        ...
        e.add_field(name="📊 !poll <question>",    value="Créer un vote 👍/👎 dans le salon",                   inline=False)
        e.add_field(name="🏷️ !roleinfo @rôle",     value="Détails d'un rôle — membres, couleur, permissions",   inline=False)
...
async def help_modo(ctx):
    ...
    if LANG == "en":
        ...
        e.add_field(name="✅ !unban <ID or name>",              value="Unban someone from the ban list",                        inline=False)
        ...
        e.add_field(name="⚠️ !warn @member [reason]",          value="Issue a warning — DM automatically sent to the member",  inline=False)
        ...
        e.add_field(name="⏱️ !slowmode [seconds]",             value="Set slowmode delay — 0 to disable",                      inline=False)
        e.add_field(name="🔒 !lock",                           value="Lock the channel — read-only for members",               inline=False)
        e.add_field(name="🔓 !unlock",                         value="Unlock the channel and restore send access",             inline=False)
        e.add_field(name="📢 !announce #channel <msg>",        value="Send a clean embed announcement *(admin)*",              inline=False)
        e.add_field(name="✏️ !setnick @member <nickname>",     value="Change a member's server nickname",                      inline=False)
    else:
        ...
        e.add_field(name="✅ !unban <ID ou nom>",               value="Débannir quelqu'un de la liste des bannis",                 inline=False)
        e.add_field(name="🔇 !exclure @membre [min] [raison]", value="Exclure temporairement — défaut 10 min, max 28 jours",      inline=False)
        e.add_field(name="🔊 !unexclure @membre",              value="Retirer immédiatement l'exclusion d'un membre",             inline=False)
        ...
        e.add_field(name="⏱️!slowmode [secondes]", value="Définir le slowmode – 0 pour désactiver", inline=False)
        e.add_field(name="🔒!lock", value="Verrouiller le salon en lecture seule pour les membres", inline=False)
        e.add_field(name="🔓!unlock", value="Déverrouiller le salon et rétablir l'envoi de messages", inline=False)
